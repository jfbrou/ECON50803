...
\begin{frame}{D'où vient la montée des inégalités intra-pays?}
   \begin{enumerate}
      ...
      \item Progrès technologique biaisé et IA
   \end{enumerate}
\end{frame}
...
\begin{frame}[t]{Érosion des institutions du marché du travail}
   ...
   \begin{itemize}
      \setlength\itemsep{0.15cm}
      \item Baisse du taux de syndicalisation
      \item N'explique qu'environ 10\,\% de l'augmentation de l'inégalité
   \end{itemize}
   \vspace{0.3cm}
   \textbf{Salaire minimum :}
   \vspace{0.15cm}
   \begin{itemize}
      \setlength\itemsep{0.15cm}
      \item Baisse du salaire minimum \textit{réel}
      \item N'explique que le bas de l'échelle salariale
   \end{itemize}
   \vspace{0.3cm}
   \textbf{Fiscalité :}
   \vspace{0.15cm}
   \begin{itemize}
      \setlength\itemsep{0.15cm}
      \item Réductions d'impôts pour les hauts revenus dans les années 1980
      \item Prévalence accrue des paradis fiscaux
   \end{itemize}
\end{frame}

\begin{frame}[t]{Mais l'État Robin des Bois a riposté}
   \framesubtitle{Les systèmes fiscaux sont devenus \textit{plus} redistributifs depuis 1990}
   \vspace{0.1cm}
   \begin{itemize}
      \setlength\itemsep{0.25cm}
      \item 7 économies avancées sur 10 ont \textbf{augmenté} la progressivité de leur fiscalité
      \item Les transferts sociaux sont passés de 17\,\% à 22\,\% du PIB
      \item Le 1\,\% le plus riche paie une part croissante de l'impôt sur le revenu (É.-U.\ : 33\,\% $\to$ 40\,\%; R.-U.\ : 22\,\% $\to$ 27\,\%)
   \end{itemize}
   \vspace{0.2cm}
   \begin{warning}
      Les inégalités \textit{avant impôt} ont augmenté, mais les inégalités \textit{après impôt} sont restées stables dans le G7. Le système fiscal a compensé --- mais les forces sous-jacentes (technologie, mondialisation) restent puissantes.
   \end{warning}
   \vspace{0.1cm}
   \mbox{}\hfill{\scriptsize\textcolor{HECgray}{\textit{The Economist}, 16 février 2026}}
\end{frame}
...
% --- 3. Progrès technologique biaisé et IA ------------------------------------

\begin{frame}{Progrès technologique biaisé}
...
\begin{frame}[t]{Déplacement vs.\ augmentation}
   \framesubtitle{Acemoglu \& Restrepo (2019)}
   ...
   \textbf{Effet de productivité / augmentation :}
   \begin{itemize}
      \setlength\itemsep{0.15cm}
      \item L'automatisation réduit les coûts $\to$ hausse de la production $\to$ nouvelles tâches pour les humains
      \item Les guichets automatiques ont \textit{réduit} le nombre de caissiers par succursale, mais les banques ont ouvert \textbf{43\,\% plus de succursales} $\to$ l'emploi total a augmenté
   \end{itemize}
\end{frame}

\begin{frame}[t]{Le danger de l'automatisation \guillemotleft~médiocre~\guillemotright}
   \textbf{Le résultat net dépend de l'équilibre entre les deux forces.}
   \vspace{0.2cm}

   Les technologies qui déplacent des travailleurs \textit{sans} gains de productivité significatifs créent le pire scénario :
   \vspace{0.15cm}
   \begin{itemize}
      \setlength\itemsep{0.15cm}
   ...
   \begin{warning}[Automatisation médiocre]
      Une technologie qui remplace des travailleurs bon marché par des machines à peine meilleures augmente les inégalités sans faire croître le gâteau.
      \mbox{}\hfill{\scriptsize\textcolor{HECgray}{Acemoglu \& Restrepo (2019)}}
   \end{warning}
\end{frame}

\begin{frame}[t]{Et l'IA\,? Deux visions}
   \small
   \begin{columns}[t]
      \begin{column}{0.47\textwidth}
         \textbf{\textcolor{HECcoral}{Vision pessimiste}}
         \vspace{0.15cm}
         \begin{itemize}
            \setlength\itemsep{0.15cm}
            \item L'IA remplace des tâches cognitives routinières à grande échelle
            \item Les gains vont surtout aux propriétaires du capital (données, modèles)
            \item Acemoglu (2024) : l'IA pourrait n'augmenter le PIB que de \textbf{1 à 2\,\%} en 10 ans
         \end{itemize}
      \end{column}
      \begin{column}{0.47\textwidth}
         \textbf{\textcolor{HECgreen}{Vision optimiste}}
         \vspace{0.15cm}
         \begin{itemize}
            \setlength\itemsep{0.15cm}
            \item L'IA \textit{démocratise} l'expertise : un technicien avec l'IA fait le travail d'un expert
            \item Crée de \textbf{nouvelles tâches} à haute valeur ajoutée
            \item Autor (2024) : l'IA pourrait \textbf{reconstruire la classe moyenne}
         \end{itemize}
      \end{column}
   \end{columns}
   \vspace{0.2cm}
   \begin{keyinsight}
      Le résultat dépend de choix de politique et de gestion : l'IA peut déplacer \textit{ou} augmenter les travailleurs. Ce n'est pas un destin technologique.
   \end{keyinsight}
\end{frame}

\begin{frame}[t]{Le paradoxe de Polanyi}
   \framesubtitle{Les limites de l'automatisation}
   \small
   \textit{\guillemotleft~We know more than we can tell.~\guillemotright} --- Michael Polanyi (1966)
   \vspace{0.15cm}

   Les tâches nécessitant un \textbf{savoir tacite} résistent à l'automatisation :
   \vspace{0.1cm}
   \begin{itemize}
      \setlength\itemsep{0.15cm}
      \item \textbf{Jugement en situation nouvelle :} un chirurgien face à une complication imprévue
      \item \textbf{Lecture du contexte :} un gestionnaire qui \guillemotleft~lit la salle~\guillemotright{} en négociation
      \item \textbf{Sens commun :} un avocat qui perçoit les incohérences d'un témoignage
   \end{itemize}
   \vspace{0.15cm}

   L'apprentissage profond contourne partiellement ce paradoxe (apprentissage par exemples plutôt que par règles), mais l'IA reste \textbf{fragile face à la nouveauté}.
   \mbox{}\hfill{\scriptsize\textcolor{HECgray}{Autor (2014)}}
\end{frame}

\begin{frame}[t]{Premiers résultats : l'IA compresse la distribution des compétences}
   \small
   \vspace{0.1cm}
   \textbf{Service à la clientèle} (Brynjolfsson, Li \& Raymond, 2023)
   \vspace{0.1cm}
   \begin{itemize}
      \setlength\itemsep{0.1cm}
      \item Assistants avec accès à un outil IA : \textbf{+14\,\%} de productivité en moyenne
      \item \textbf{+34\,\%} pour les employés novices --- effet quasi nul pour les experts
      \item L'IA diffuse les meilleures pratiques des meilleurs agents vers les débutants
   \end{itemize}
   \vspace{0.2cm}

   \textbf{Tâches de rédaction} (Noy \& Zhang, 2023)
   \vspace{0.1cm}
   \begin{itemize}
      \setlength\itemsep{0.1cm}
      \item Temps pour compléter la tâche : \textbf{$-$40\,\%}
      \item Qualité évaluée en aveugle : \textbf{+18\,\%}
      \item Les plus grands gains pour les rédacteurs les moins habiles
   \end{itemize}
   \vspace{0.2cm}

   \textbf{Programmation} (Peng et al., 2023)
   \vspace{0.1cm}
   \begin{itemize}
      \setlength\itemsep{0.1cm}
      \item Développeurs avec GitHub Copilot : tâche complétée \textbf{56\,\% plus rapidement}
   \end{itemize}
\end{frame}

\begin{frame}[t]{Adoption de l'IA : encore tôt, mais en accélération}
   \textbf{Taux d'adoption par les entreprises (États-Unis) :}
   \vspace{0.2cm}
   \begin{itemize}
      \setlength\itemsep{0.2cm}
      \item 2023 : \textbf{3,7\,\%} des entreprises utilisaient l'IA (Census Bureau)
      \item 2025 : \textbf{18\,\%} --- et 22\,\% prévoient l'adopter dans les 6 prochains mois
      \item Grandes entreprises (250+) : \textbf{27\,\%}
   \end{itemize}
   \vspace{0.3cm}

   \textbf{Exposition des emplois (FMI, 2024) :}
   \vspace{0.2cm}
   \begin{itemize}
      \setlength\itemsep{0.2cm}
      \item $\sim$\textbf{40\,\%} des emplois mondiaux sont \guillemotleft~exposés~\guillemotright{} à l'IA
      \item \textbf{60\,\%} dans les économies avancées (tâches cognitives)
      \item La moitié de ces emplois exposés pourraient \textit{bénéficier} de l'IA; l'autre moitié risque le déplacement
   \end{itemize}
\end{frame}
...
\begin{frame}[t]{Leçons de l'histoire}
   ...
   \begin{itemize}
      \setlength\itemsep{0.2cm}
      \item \textbf{Guichets automatiques} (années 1970--90) : le nombre de caissiers par succursale a baissé, mais l'emploi total a \textit{augmenté} car les banques ont ouvert plus de succursales
   ...
   \begin{bizimplication}
      L'automatisation élimine des \textit{tâches}, rarement des \textit{métiers} entiers. Elle crée aussi de nouvelles tâches --- mais la transition peut être longue et douloureuse.
   \end{bizimplication}
\end{frame}
...
\begin{frame}[t]{Faut-il s'inquiéter de la montée des inégalités?}
   \textbf{Inégalités et incitatifs :}
   \vspace{0.2cm}
   \begin{itemize}
      \setlength\itemsep{0.2cm}
      \item Un certain niveau d'inégalité est nécessaire au fonctionnement des incitatifs
      \item Inégalité des résultats $\neq$ inégalité des chances
   \end{itemize}
   \vspace{0.4cm}
   ...
   \begin{itemize}
      \setlength\itemsep{0.2cm}
      \item L'inégalité des résultats est associée à une plus faible \textbf{mobilité économique}
      \item L'inégalité des chances nous prive de nos \textbf{meilleurs talents}
   \end{itemize}
\end{frame}
...
\begin{frame}{La \guillemotleft~Courbe de Gatsby~\guillemotright}
   \begin{figure}
      \centering
      \includegraphics[width=0.95\textwidth, height=0.78\textheight, keepaspectratio]{gatsby.png}
   \end{figure}
\end{frame}
