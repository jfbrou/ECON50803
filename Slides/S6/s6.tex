...
\begin{frame}[t]{Les tarifs et le modèle S-I}
   ...
   \begin{columns}[T]
      \begin{column}{0.47\textwidth}
         \textbf{Effet sur l'épargne ($S$)\,?}
         \begin{itemize}
            \setlength\itemsep{0.1cm}
            \item Revenus tarifaires $\to$ $S_G \uparrow$\,? Pas si les revenus financent des \textbf{baisses d'impôts}
            \item Prix à la consommation $\uparrow$ $\to$ revenu réel $\downarrow$ $\to$ $S_P$ \textbf{diminue}
            \item Effet net sur $S$ : $\approx 0$
         \end{itemize}
      \end{column}
      \begin{column}{0.47\textwidth}
         \textbf{Effet sur l'investissement ($I$)\,?}
         \begin{itemize}
            \setlength\itemsep{0.1cm}
            \item $I$ dépend des rendements anticipés et des taux, pas des tarifs
            \item L'incertitude peut réduire $I$, mais seulement via une \textbf{récession}
            \item Effet net sur $I$ : $\approx 0$
         \end{itemize}
      \end{column}
   \end{columns}
   \vspace{0.15cm}
   \centering
   {\footnotesize\textcolor{HECgray}{Feldstein et Shultz (2017) : \guillemotleft~Si un pays consomme plus qu'il ne produit, il doit importer plus qu'il n'exporte. Ce n'est pas une arnaque, c'est de l'arithmétique.~\guillemotright}}
\end{frame}
