...
\begin{frame}[t]{L'\'{e}conomie peut-elle se corriger seule\,?}
   \vspace{0.2cm}
   \begin{defbox}[Rappel de la S\'{e}ance 4]
      Les chocs de demande et d'offre \'{e}loignent $Y$ de $Y_{\text{POT}}$. Mais \textbf{AD} a une pente n\'{e}gative (effet de richesse) et \textbf{AS} a une pente positive (salaires rigides \`{a} court terme). Ces pentes cr\'{e}ent-elles un m\'{e}canisme de retour automatique\,?
   \end{defbox}
   \vspace{0.3cm}
   \begin{itemize}
      \setlength\itemsep{0.3cm}
      \item La question centrale : l'\'{e}conomie a-t-elle \textbf{besoin} d'une intervention, ou les prix et salaires peuvent-ils faire le travail\,?
      \item Si oui, \textbf{par quel m\'{e}canisme}\,? Et \`{a} quelle vitesse\,?
   \end{itemize}
\end{frame}

\begin{frame}[t]{Le m\'{e}canisme d'ajustement naturel}
   \small
   \vspace{0.1cm}
   AS a une pente positive parce que les \textbf{salaires nominaux sont rigides} \`{a} court terme. Mais ils ne le sont pas \textbf{pour toujours} :
   \vspace{0.15cm}
   \begin{columns}[T]
      \begin{column}{0.47\textwidth}
         ...
         \begin{enumerate}
            ...
            \item Travailleurs perdent du pouvoir de n\'{e}gociation
            \item Salaires nominaux finissent par $\downarrow$
            \item Co\^{u}ts de production $\downarrow$
            \item[\textcolor{HECnavy}{$\Rightarrow$}] \textbf{AS se d\'{e}place vers la droite}
         \end{enumerate}
      \end{column}
      \begin{column}{0.47\textwidth}
         ...
         \begin{enumerate}
            \setlength\itemsep{0.08cm}
            \item March\'{e} du travail tendu
            \item Travailleurs n\'{e}gocient des hausses
            \item Salaires nominaux $\uparrow$
            \item Co\^{u}ts de production $\uparrow$
            \item[\textcolor{HECcoral}{$\Rightarrow$}] \textbf{AS se d\'{e}place vers la gauche}
         \end{enumerate}
      \end{column}
   \end{columns}
   \vspace{0.15cm}
   \begin{keyinsight}
      C'est le \textbf{niveau des salaires nominaux} qui d\'{e}termine la position de AS. Quand les salaires s'ajustent, AS se d\'{e}place --- et l'\'{e}conomie revient vers $Y_{\text{POT}}$.
   \end{keyinsight}
\end{frame}
   ...
   \begin{columns}[T]
      ...
      \begin{column}{0.42\textwidth}
         ...
         \begin{enumerate}
            ...
            \item Choc n\'{e}gatif $\to$ AD se d\'{e}place \`{a} gauche
            \item Nouvel \'{e}quilibre $E_1$ : production $\downarrow$, prix $\downarrow$
...
\begin{frame}[t]{Choc de demande n\'{e}gatif : l'ajustement naturel}
   ...
   \begin{columns}[T]
      ...
      \begin{column}{0.42\textwidth}
         ...
         \end{enumerate}
         \vspace{0.2cm}
         \textcolor{HECgray}{\footnotesize M\^{e}me niveau de production, mais prix plus bas.}
      \end{column}
   \end{columns}
\end{frame}
...
\begin{frame}[t]{Choc de demande positif : le choc}
   ...
   \begin{columns}[T]
      ...
      \begin{column}{0.42\textwidth}
         ...
         \begin{enumerate}
            \setlength\itemsep{0.15cm}
            \item Choc positif $\to$ AD se d\'{e}place \`{a} droite
            \item Nouvel \'{e}quilibre $E_1$ : production $\uparrow$, prix $\uparrow$
...
\begin{frame}[t]{Choc de demande positif : l'ajustement naturel}
   ...
   \begin{columns}[T]
      ...
      \begin{column}{0.42\textwidth}
         ...
         \begin{enumerate}
            \setlength\itemsep{0.1cm}
            \item $Y_1 > Y_{\text{POT}}$ $\to$ march\'{e} du travail tendu
            \item Travailleurs n\'{e}gocient des hausses
         ...
         \end{enumerate}
         \vspace{0.2cm}
         \textcolor{HECgray}{\footnotesize M\^{e}me niveau de production, mais prix plus \'{e}lev\'{e}s.}
      \end{column}
   \end{columns}
\end{frame}

\begin{frame}[t]{Pourquoi ne pas laisser faire\,?}
   \vspace{0.2cm}
   \begin{columns}[T]
      \begin{column}{0.47\textwidth}
         ...
         \begin{itemize}
            \setlength\itemsep{0.15cm}
            \item Prix : dur\'{e}e m\'{e}diane de 8 mois entre deux changements
            \item Salaires : r\'{e}vis\'{e}s $\sim$1 fois par an (conventions, contrats)
   ...
   \vspace{0.3cm}
   \begin{keyinsight}
      C'est le fondement de la \textbf{politique contracyclique} : intervenir pour acc\'{e}l\'{e}rer le retour vers $Y_{\text{POT}}$.
   \end{keyinsight}
\end{frame}
...
\begin{frame}{2020--2022 : un cas d'\'{e}tude grandeur nature}
   ...
   \centering
   \begin{tikzpicture}
      \node[casebox=HECnavy] (a) at (0,0) {\textbf{2020}\\Pand\'{e}mie\\$Y \downarrow\downarrow$};
      \node[casebox=HECgreen] (b) at (4.5,0) {\textbf{2020--21}\\Relance massive\\BdC $\downarrow$ taux \`{a} 0.25\,\%\\PCU, QE};
      \node[casebox=HECcoral] (c) at (9,0) {\textbf{2022}\\Inflation \`{a} 8\,\%\\BdC $\uparrow$ taux\\de 0.25 \`{a} 5\,\%};
      \draw[casearr=HECnavy] (a) -- (b);
      \draw[casearr=HECgreen] (b) -- (c);
   \end{tikzpicture}
   \vspace{0.3cm}
   \begin{keyinsight}
      La politique contracyclique a \'{e}vit\'{e} une d\'{e}pression, mais la relance \'{e}tait-elle \textbf{trop g\'{e}n\'{e}reuse}\,? Comprendre ces outils et leurs limites, c'est le c\oe ur de cette s\'{e}ance.
   \end{keyinsight}
\end{frame}

\begin{frame}[t]{Deux leviers, deux institutions}
   \vspace{0.3cm}
   \begin{columns}[T]
      \begin{column}{0.47\textwidth}
         \centering
...
\begin{frame}[t]{Stabilisateurs automatiques}
   \small
   \vspace{0.2cm}
   \begin{defbox}
      Les \textbf{stabilisateurs automatiques} sont des m\'{e}canismes budg\'{e}taires qui s'activent \textit{sans} nouvelle l\'{e}gislation quand l'\'{e}conomie ralentit.
   \end{defbox}
   \vspace{0.15cm}
   \begin{columns}[T]
      \begin{column}{0.47\textwidth}
         \textbf{En r\'{e}cession :}
         \begin{itemize}
            \setlength\itemsep{0.1cm}
            \item Revenus d'imp\^{o}t $\downarrow$ automatiquement
            \item Prestations d'assurance-emploi $\uparrow$
            \item Aide sociale $\uparrow$
         \end{itemize}
         \vspace{0.1cm}
         $\to$ Soutient AD sans vote au Parlement
      \end{column}
      \begin{column}{0.47\textwidth}
         \textbf{En surchauffe :}
         \begin{itemize}
            \setlength\itemsep{0.1cm}
            \item Revenus d'imp\^{o}t $\uparrow$ (barèmes progressifs)
            \item Prestations sociales $\downarrow$
         \end{itemize}
         \vspace{0.1cm}
         $\to$ Fr\`{e}ne AD automatiquement
      \end{column}
   \end{columns}
\end{frame}

\begin{frame}{Le dilemme du choc d'offre}
   \vspace{0.2cm}
   \small
   \textbf{Rappel (S\'{e}ance 4) :} face \`{a} un choc d'offre (ex.\ : Iran 2026), la politique contracyclique est \textbf{pi\'{e}g\'{e}e} :
   \vspace{0.2cm}
   \begin{itemize}
      \setlength\itemsep{0.2cm}
      \item \textbf{Stimuler AD} $\to$ att\'{e}nue la r\'{e}cession, mais \textbf{aggrave l'inflation}
      \item \textbf{Resserrer AD} $\to$ combat l'inflation, mais \textbf{approfondit la r\'{e}cession}
      \item \textbf{Ne rien faire} $\to$ accepter la stagflation temporaire
   \end{itemize}
   \vspace{0.2cm}
   \begin{warning}[Stagflation = dilemme sans bonne r\'{e}ponse]
      Face \`{a} un choc d'offre, les d\'{e}cideurs doivent choisir quel mal combattre en priorit\'{e}. Il n'y a pas de solution indolore.
   \end{warning}
\end{frame}
