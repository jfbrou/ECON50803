...
% ---------- Modèle d'offre et de demande de travail (Q9--) ----------
...
\qcm{Les É.-U.\ imposent des tarifs qui réduisent fortement les commandes reçues par les exportateurs canadiens de bois d'{\oe}uvre. À court terme, dans le marché du travail de ce secteur, quel diagnostic est le plus juste\,?}
\choix{
   \item $N^D \leftarrow$, donc $N \downarrow$ et $w \downarrow$
   \item $N^S \leftarrow$, donc $N \downarrow$ et $w \uparrow$
   \item $N^D \rightarrow$, donc $N \uparrow$ et $w \uparrow$
   \item $N^S \rightarrow$, donc $N \uparrow$ et $w \downarrow$
}

\begin{reponse}
\textbf{(a)} Les tarifs réduisent la demande étrangère pour le produit canadien $\to$ les entreprises du secteur veulent produire moins $\to$ elles demandent moins de travailleurs. C'est donc un déplacement de la \textbf{demande de travail} ($N^D$) vers la \textbf{gauche}. Le nouvel équilibre se trouve en descendant le long de la courbe d'offre : l'emploi baisse ($N \downarrow$) et le salaire réel baisse ($w \downarrow$). C'est exactement le type d'identification rapide demandé à l'examen.
\end{reponse}

\qcm{Un programme spécial de visas augmente rapidement le nombre de travailleurs disponibles dans la restauration, alors que la demande de repas reste inchangée. À court terme, le modèle prédit :}
\choix{
   \item $N^D \leftarrow$, donc $N \downarrow$ et $w \downarrow$
   \item $N^S \leftarrow$, donc $N \downarrow$ et $w \uparrow$
   \item $N^D \rightarrow$, donc $N \uparrow$ et $w \uparrow$
...
\begin{reponse}
\textbf{(d)} Le choc porte ici sur la \textbf{disponibilité de travailleurs}, pas sur la demande pour les repas. La courbe d'\textbf{offre de travail} ($N^S$) se déplace donc vers la \textbf{droite}. À demande de travail inchangée, l'emploi augmente ($N \uparrow$) et le salaire réel baisse ($w \downarrow$). C'est la logique de court terme déjà vue avec l'immigration et la hausse de participation.
\end{reponse}
...
\vfi{Si une guerre commerciale réduit les débouchés d'un secteur exportateur canadien, le choc immédiat sur le marché du travail du secteur se représente par un déplacement de $N^D$ vers la gauche.}

\begin{reponse}
\textbf{Vrai.} Une baisse des débouchés réduit la production souhaitée des entreprises $\to$ elles ont besoin de moins de travailleurs à chaque niveau de salaire. La courbe de \textbf{demande de travail} ($N^D$) se déplace donc vers la \textbf{gauche}. À court terme, le modèle prédit une baisse de l'emploi et du salaire réel dans le secteur touché. C'est un choc de \textbf{demande de travail}, pas un choc d'offre de travail.
\end{reponse}

\vfi{Si, dans un secteur, un choc négatif sur la demande de biens réduit $N^D$ tandis qu'une vague de retraites réduit aussi $N^S$, l'effet sur l'emploi est ambigu.}

\begin{reponse}
\textbf{Faux.} Les deux déplacements poussent \textbf{l'emploi} dans le même sens : une baisse de $N^D$ réduit l'emploi, et une baisse de $N^S$ réduit aussi l'emploi. L'effet sur $N$ est donc \textbf{certainement négatif}. En revanche, l'effet sur le salaire réel est \textbf{ambigu} : $N^D \leftarrow$ tire $w$ vers le bas, tandis que $N^S \leftarrow$ tire $w$ vers le haut. C'est exactement le type de raisonnement combinant deux chocs que l'examen peut demander.
\end{reponse}
...
\shortq{Une entreprise de transport routier observe trois changements en 2026 : (i) une vague de retraites réduit le nombre de chauffeurs expérimentés disponibles; (ii) un boom de la construction augmente la demande de transport de matériaux; (iii) un programme accéléré de permis de travail augmente rapidement le nombre de nouveaux chauffeurs disponibles.}

\begin{enumerate}[label=(\alph*)]
   \item Pour chacun des trois changements pris séparément, indiquez s'il déplace $N^D$ ou $N^S$, dans quel sens, et l'effet attendu à court terme sur l'emploi $N$ et le salaire réel $w$.
   \item Si les changements (i) et (ii) se produisent simultanément, qu'est-ce qui est certain pour le salaire réel\,? Qu'est-ce qui devient ambigu pour l'emploi\,?
   \item Si les changements (ii) et (iii) se produisent simultanément, qu'est-ce qui est certain pour l'emploi\,? Qu'est-ce qui devient ambigu pour le salaire réel\,?
   \item Au-delà du seul taux de chômage, quel indicateur du marché du travail la direction devrait-elle suivre pour mieux juger si le problème principal est une rareté de main-d'{\oe}uvre plutôt qu'une faiblesse de la demande\,?
\end{enumerate}

\begin{reponse}
\textbf{(a)}
\begin{itemize}
   \item \textbf{(i) Retraites :} déplacement de \textbf{$N^S$ vers la gauche}. Il y a moins de chauffeurs disponibles $\to$ $N \downarrow$ et $w \uparrow$.
   \item \textbf{(ii) Boom de la construction :} déplacement de \textbf{$N^D$ vers la droite}. Les entreprises veulent transporter davantage de matériaux $\to$ $N \uparrow$ et $w \uparrow$.
   \item \textbf{(iii) Nouveaux permis de travail :} déplacement de \textbf{$N^S$ vers la droite}. Davantage de chauffeurs sont disponibles $\to$ $N \uparrow$ et $w \downarrow$.
\end{itemize}

\textbf{(b)} Avec (i) et (ii), les deux chocs poussent le \textbf{salaire réel à la hausse} : rareté de chauffeurs d'un côté, demande plus forte de l'autre. En revanche, l'effet sur l'\textbf{emploi} est \textbf{ambigu} : le choc d'offre tend à faire baisser $N$, alors que le choc de demande tend à le faire augmenter.

\textbf{(c)} Avec (ii) et (iii), les deux chocs poussent l'\textbf{emploi à la hausse} : plus de demande de transport et plus de chauffeurs disponibles. En revanche, l'effet sur le \textbf{salaire réel} devient \textbf{ambigu} : le boom de la construction pousse $w$ vers le haut, alors que l'arrivée de nouveaux chauffeurs pousse $w$ vers le bas.

\textbf{(d)} La direction devrait aussi suivre le \textbf{taux de participation}. Si le chômage est faible \textbf{et} que la participation reste faible ou recule, cela peut signaler une vraie rareté de main-d'{\oe}uvre. À l'inverse, une faiblesse de la demande s'accompagnera plus naturellement d'un emploi en baisse et d'une pression plus faible sur les salaires.
\end{reponse}
